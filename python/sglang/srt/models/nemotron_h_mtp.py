...
class NemotronHMTPMoEDecoderLayer(NemotronHMoEDecoderLayer):
    def __init__(
        ...
        self.has_start_projections = has_start_projections
        self.has_end_norm = has_end_norm
        # Mirror NemotronHAttentionDecoderLayer's dp-attention handling: the MTP draft
        # pattern is [attention, moe], so this MoE layer consumes the preceding attention
        # layer's attn-TP-partial output and must all-reduce it across the attn-TP group
        # before the MoE prepare_mlp gather. The base NemotronHMoEDecoderLayer omits this
        # because in the target hybrid pattern a MoE layer never directly follows an attn
